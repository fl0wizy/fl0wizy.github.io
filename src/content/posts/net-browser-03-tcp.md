...
이 글에서는 IP가 포기한 보장을 TCP가 어떻게 복구하는지, 그리고 복구하지 **않은** 한 가지가 무엇인지를 정리한다. 마지막 절의 "메시지 경계 없음"은 이 시리즈 전체에서 가장 자주 되돌아올 성질이다.
...
## 6. TCP의 성질을 직접 무기로 쓰는 경우

대부분의 공격은 애플리케이션 로직을 노리지만, TCP의 물리적 성질을 그대로 프리미티브로 쓰는 기법이 있다.
...
이 기법이 성립하는 이유는 6편에서 다룰 프로토콜 세부가 아니라, 3편에서 본 성질이다. **스트림에는 경계가 없고, 세그먼트 경계는 보내는 쪽이 조작할 수 있다.**

---

## 다음 편

다음 편은 주소 이전의 이름을 본다. HOSTS.TXT 한 파일이 왜 무너졌는지, TTL이라는 숫자가 어떤 타협이었는지, 그리고 "이름에서 주소로의 변환은 시간이 지나면 답이 바뀔 수 있다"는 성질이 어떻게 브라우저 보안 모델을 우회하는 통로가 되는지다.
