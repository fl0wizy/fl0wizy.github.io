...
이 글은 AI 하네스 시리즈의 열 번째 편이다. 강제력의 3단계인 요청, 집행, 격리를 먼저 보고, prompt injection이 왜 아직 안 풀린 문제인지, audit log와 운영 지표로 무엇을 봐야 하는지, 그리고 하네스도 낡는다는 문제까지 정리한다.

---

## 1. 강제력의 3단계

이 편의 핵심 문장은 이것이다. **매번 일어나야 하는 일은 지침이 아니라 훅에 있어야 한다.** 가스레인지 옆에 "불조심"이라고 써 두는 것과, 일정 시간이 지나면 자동으로 가스를 차단하는 장치는 다르다. 둘 다 필요하지만 역할이 다르다.
...
Prompt injection은 외부 문서나 웹페이지가 AI에게 악성 지시를 심어, 원래 사용자의 의도와 다르게 행동하게 만드는 공격이다. 처음 배우는 사람에게는 **AI가 읽는 문서 속에 가짜 상사의 지시가 숨어 있는 것**이라고 설명할 수 있다.
...
운영자가 주기적으로 던질 질문은 세 가지다. 이 규칙은 아직 필요한가(낡은 규칙은 새 모델의 능력을 막을 수 있다). 이 도구는 안전하게 제한되어 있는가(위험한 도구는 권한, 승인, 감사 로그가 필요하다). 이 실패는 테스트나 훅으로 자동 감지되는가(사람의 기억보다 시스템 검사가 안정적이다).
...
투명성을 "모든 로그 공개"로 오해하면 안 된다. 요약을 먼저 보여 주고, 상세를 펼칠 수 있게 하고, 위험 지점을 중심에 두는 **점진적 공개**가 맞다.
