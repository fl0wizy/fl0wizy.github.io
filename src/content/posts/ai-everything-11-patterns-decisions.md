...
도구 개수와 에이전트 성능은 비례하지 않는다. 첫 번째 역발상이 여기에 있다. 도구를 많이 붙이면 더 똑똑해질 것 같지만, 도구가 늘어나면 선택 부담과 컨텍스트 부담도 함께 늘어난다. 버튼이 80개인 리모컨은 강력해 보이지만 기본 작업조차 불편해진다.

Next.js를 만든 프론트엔드 인프라 회사 Vercel은 내부 text-to-SQL 에이전트 **d0**를 운영했다. Slack에서 자연어로 질문하면 분석 인프라에 SQL을 날려 답을 얻는 데이터 에이전트다. 처음에는 여러 전문 도구, 무거운 프롬프트 엔지니어링, 세심한 컨텍스트 관리, hand-coded 로직으로 정교하게 만들었다. 이후 접근을 바꿨다. 많은 전문 도구를 제거하고, 에이전트를 파일시스템 중심 구조로 단순화하고, Claude가 데이터 지표 정의를 관리하는 도구인 Cube의 semantic layer를 구성하는 YAML, Markdown, JSON 파일을 직접 읽으며 `grep`, `cat`, `find`, `ls` 같은 표준 Unix 도구로 탐색하게 했다.
...
ReAct(Reasoning + Acting, 생각하고 행동하기를 번갈아 하는 에이전트 방식을 제안한 2022년 논문이자 그 패턴 이름)와 계획 후 실행 방식 사이에는 고정된 우열이 없다. 두 번째 역발상이 여기에 있다. ReAct는 튜토리얼에 자주 등장하고 똑똑해 보인다. 하지만 매 단계 새로 생각한다는 것은 **매 단계 비용과 지연이 생긴다**는 뜻이다. 배달로 치면, 매 집 앞에서 다음엔 어디로 갈지를 다시 생각하면 유연하지만 느리다. 경로를 한 번 짜고 차례대로 돌면 훨씬 빠르다.

도구 호출들의 의존 관계를 그래프로 먼저 계획해 병렬 실행하는 기법을 제안한 LLMCompiler 연구가 이를 수치로 보여 준다. 여러 함수 호출이 필요한 작업에서 계획과 의존성을 먼저 세우고 병렬 실행하면, ReAct 대비 최대 3.7배 지연시간 개선, 최대 6.7배 비용 절감이 가능했다. (Kim et al., *An LLM Compiler for Parallel Function Calling*)
