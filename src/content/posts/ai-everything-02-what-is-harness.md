...
이 글은 하네스를 공부하며 정리한 개념 노트다. "하네스"라는 말이 자동차 배선에서 소프트웨어 테스트를 거쳐 AI로 이어진 계보, 모델과 하네스가 한 턴에서 각각 어디에 있는지, 하네스의 최소 구성요소 5개와 제품급 구성요소 12개, 하네스가 모델 바깥에 두는 외부 구성요소(메모리·스킬·프로토콜)를 정리한다.
