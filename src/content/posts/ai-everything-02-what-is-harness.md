...
이 글은 하네스를 공부하며 정리한 개념 노트다. "하네스"라는 말이 자동차 배선에서 소프트웨어 테스트를 거쳐 AI로 이어진 계보, 하네스의 최소 구성요소 5개와 제품급 구성요소 12개, 하네스가 모델 바깥으로 꺼내 두는 것(메모리·스킬·프로토콜)을 정리한다.
...
LangChain은 더 과감하게 정의한다. **모델이 아니라면, 그것은 하네스다.** 시스템 프롬프트, 도구, MCP, 파일시스템, 샌드박스, 오케스트레이션 로직, hook/middleware가 전부 하네스다. 과감해 보이지만 이 정의는 책임의 위치를 바꿔준다는 점에서 유용하다.
...
에이전트를 "모델에 도구 몇 개를 붙인 것"으로 생각하기 쉽지만, 하네스 관점의 구조는 오히려 반대에 가깝다. **모델 자체는 가능한 한 얇게 두고, 모델이 매번 스스로 들고 있을 필요가 없는 기억·절차·규칙을 모델 바깥에 둔다.** 하네스는 이 바깥 요소들을 실행 시점에 조합한다.
