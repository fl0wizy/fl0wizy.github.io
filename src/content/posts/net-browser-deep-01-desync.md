...
6편 §2가 한 줄로 접어 둔 변형들을 여기서 편다. 이름이 여러 개라 목록으로 외우기 쉬운데, 실제로는 축이 하나다. **연결이 재사용되고, 그 위에서 "메시지의 끝"에 대한 판단이 둘로 갈린다.**
...
표가 담는 것은 프론트엔드와 백엔드가 메시지를 다르게 끊는 다섯 조합까지이고, 뒤에서 볼 client-side desync와 pause-based desync는 이 두 열로 갈리지 않는다.
...
15편의 분류로 보면 이 편 전체가 원형 A(경계 불일치)의 가장 순수한 사례다.

---

## 2. CL.0 – 규격을 지킨 요청의 변형

표의 네 번째 행인 CL.0은 성격이 다르다. 백엔드가 `Content-Length`를 무시하는, 즉 본문을 아예 읽지 않는 상황을 노린다. PortSwigger의 연구는 이런 상황이 왜 흔한지를 이렇게 정리한다.
...
## 3. client-side desync – 브라우저 연결의 오염
...
**early-response gadget**은 본문이 도착하기 전에 백엔드가 응답을 내놓게 만드는 경로다. 예를 들어 일부 서버에서 예약된 파일명을 가리키는 경로는 본문을 기다리지 않고 즉시 오류를 반환한다. 그 응답이 나오면 교착이 풀리고, 이후 double-desync라 불리는 변형을 통해 악용 가능한 CL.0(§2) 상태로 전환된다.
...
여기 정리한 내용은 전부 공개된 연구이고, 실제 점검은 인가된 범위에서만 한다. 이 기법들은 **성공하면 다른 사용자의 요청을 망가뜨린다.** 접두사가 남은 연결에 무관한 사용자의 요청이 도착하면 그 사용자는 깨진 응답을 받거나 요청이 실패한다. 운영 중인 서비스에서 부주의하게 시도하면 실제 장애를 만든다.
...
되돌아갈 자리는 본문의 끝을 정하는 규칙을 세운 [6편 §2](/post/net-browser-06-http1)와, 연결 재사용을 배달 경로로 놓은 같은 편 §3이다.

같은 형태를 다른 계층에서 보려면 렌더 비용의 차이가 그대로 관측 채널이 되는 경로를 다룬 [심화 02편](/post/net-browser-deep-02-pipeline)으로 간다.
