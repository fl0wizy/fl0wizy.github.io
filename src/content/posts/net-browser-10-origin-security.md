...
이 글에서는 브라우저의 접근 제어 단위인 출처(origin)를 정리한다. 이 계층의 특징은 하나의 일관된 원칙에서 연역된 설계가 아니라, **사고가 터질 때마다 하나씩 덧붙은 지층**이라는 점이다. 각 방어가 어떤 사고에 대한 대응이었는지를 붙이면 이름들의 배치가 정리된다.

![출처와 방어의 지층](/images/net-browser/10-origin.svg)

---

## 1. 출처의 정의와, 그와 다른 기준을 쓰는 쿠키
...
다음 파트는 서버 쪽으로 넘어간다. 정적 HTML에서 SPA로, 다시 서버 렌더링으로 돌아온 프런트엔드의 진화가 각각 어떤 문제를 풀려는 시도였는지부터 본다.
