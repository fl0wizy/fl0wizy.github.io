...
이 글은 멀티에이전트를 공부하며 정리한 개념 노트다. 멀티에이전트의 조건, Subagent와 Agent Teams의 차이, 역할 이름이 아니라 문맥 경계로 나누는 이유, 그리고 멀티에이전트를 쓰지 말아야 할 때까지 한 번에 훑어본다.

---

## 1. 모델이 섞여야 멀티에이전트인가

멀티에이전트가 되려면 모델이 섞여야 할까. 하나의 세션에서 10개 서브에이전트를 오케스트레이션하는데 그 10개가 모두 Opus와만 통신한다면 멀티에이전트가 맞는가 하는 의문이 생길 수 있다.

결론부터 말하면 **멀티에이전트가 맞고, 모델이 섞일 필요는 전혀 없다.** 멀티에이전트를 정의하는 것은 모델 다양성이 아니라 네 가지다.
