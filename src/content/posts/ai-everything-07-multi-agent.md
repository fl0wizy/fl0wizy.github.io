...
멀티에이전트는 각자 독립된 컨텍스트 창과 실행 루프를 가진 에이전트 여럿이 하나의 목표를 나누어 처리하는 구조다. 여기서 자주 나오는 의문이 모델 다양성이다. 하나의 세션에서 10개 서브에이전트를 오케스트레이션하는데 그 10개가 모두 Claude의 상위 모델 등급인 Opus와만 통신한다면 멀티에이전트가 맞는가 하는 물음이다.
