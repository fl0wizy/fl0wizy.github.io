...
이 글은 AI 에이전트의 장시간 실행과 메모리를 공부하며 정리한 개념 노트다.
...
LangChain의 하네스 글이 짚는 포인트도 있다. 긴 작업에서 에이전트는 컨텍스트가 복잡해지면 스스로 이제 마무리하겠다고 판단할 수 있다. 이때 하네스가 바로 종료시키지 않고, 새롭고 깨끗한 세션을 열어 원래 목표와 진행 파일, 남은 기능 목록을 다시 읽게 만들 수 있다. 퇴근하려는 의사에게 인수인계 차트에 빠진 환자는 없는지 확인한 뒤 다음 근무자에게 넘기게 하는 것과 같다.
