...
이 글은 AI를 공부하며 정리한 시리즈의 전체 지도다. 같은 모델을 쓰는데 왜 어떤 사람은 안정적으로 결과를 내고 어떤 사람은 매번 흔들리는가, 이 질문 하나에서 시리즈 전체가 출발한다.

---

## 1. 이 시리즈가 답하려는 질문

이 질문을 따라가다 보면 AI를 보는 눈이 바뀐다. "GPT가 나을까, Claude가 나을까"라는 비교에서 "이 모델이 일하는 환경은 제대로 설계되어 있는가"라는 질문으로 내려간다. 그 환경의 이름이 하네스(harness)다.
...
AI가 실패했을 때 대부분의 사람은 먼저 모델을 의심한다. 하네스 관점은 그 의심을 **고칠 수 있는 대상**으로 바꿔준다.
...
부록으로 [내 학습 노트 팩트체크](/post/ai-study-notes-verified)가 있다. 15개 항목을 검증해 틀린 것 2개, 보정 6개를 찾았다.
