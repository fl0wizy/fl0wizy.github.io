...
그 외에 흔히 섞이는 이름들도 층위를 나눠 보면 정리된다. ChatGPT는 제품/표면이고 그 뒤의 모델이 GPT 계열이다. Claude 앱도 마찬가지로 제품/표면이고 모델은 Claude 계열이다. Kimi는 Moonshot AI의 모델이다. Perplexity는 검색 특화 제품/하네스로, 내부에서 여러 모델을 혼용한다. Cursor는 에디터형 하네스라 여러 모델을 골라 끼울 수 있고, LangGraph나 CrewAI는 하네스를 만드는 프레임워크다.
...
이건 하네스가 결과를 만든다는 명제의 가장 깨끗한 실증 사례다.
...
동네 카페로 비유하면 운영 수칙(위생 기준·마감 절차), 레시피 카드(음료 제조 순서), 직원 분담(원가 조사와 포스터 문구), 별도 작업대(신메뉴 실험), 자동 체크리스트(마감 시간)의 합이다. 이 전체가 카페 운영 하네스다.

OpenAI의 Responses API는 모델 응답, 도구 호출, built-in tools, function calling, MCP 연결을 한 흐름으로 구성한다. 개인 비서 사무실에 빗대면 Responses API는 모든 요청이 처음 들어오는 접수 창구, function calling은 캘린더·메일·예약 앱을 실제로 여는 행동, built-in tools는 검색·파일 찾기·계산 같은 기본 도구, Handoff는 법무·회계처럼 전문 담당자에게 넘기기, Guardrail은 결제·외부 발송·민감 정보 처리 전에 멈추는 확인 규칙, Tracing은 누가 어떤 도구를 썼는지 남기는 업무 일지, Eval은 비서 업무가 정확했는지 주기적으로 확인하는 점검표다.
...
Google은 **관찰 가능성**을 하네스의 생명선으로 놓는다. MCP와 A2A 표준 레이어가 대표 구조이고, 복잡한 하네스는 수명이 짧다고 경계하며, 표준화를 통한 멀티에이전트 민주화를 다음 방향으로 잡는다.
...
새로 등장한 개념으로 **Harness Debt**가 있다. 하네스 파일이 stale해지면서 쌓이는 부채인데, 기존 기술 부채만큼 심각하지만 아직 측정 도구가 없다.
