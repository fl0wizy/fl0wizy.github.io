...
여기까지가 부품 목록이라면, 이 절은 요청 하나가 실제로 어느 기계에서 무슨 일을 겪는지를 따라간다. 실무에서 가장 자주 헷갈리는 두 지점이 있다. 문자열이 토큰으로 번역되는 자리, 그리고 "웹 검색해줘"가 실제로 실행되는 자리다.

### 토큰화가 일어나는 자리

**토큰화(tokenization)** 는 문자열을 모델이 처리할 수 있는 토큰 ID 배열로 변환하는 과정이고, 이 변환은 하네스가 아니라 **제공자 서버**에서 일어난다. 하네스가 조립해 보내는 요청 본문에는 토큰 ID가 하나도 없다. CLAUDE.md 내용, 대화 이력, 도구 스키마가 전부 문자열인 채로 JSON에 담겨 HTTPS로 나간다.

그래서 하네스가 화면에 띄우는 "컨텍스트 62% 사용" 같은 표시는 셋 중 하나다. 직전 응답의 `usage` 필드에 서버가 적어 보낸 사후 값이거나, 별도의 토큰 계산 엔드포인트에 물어본 값이거나, 클라이언트가 자체 규칙으로 계산한 근사치다. 어느 쪽이든 실시간 정확값은 아니다.

구현체마다 사정이 다른 것은 토크나이저 공개 여부다.

Anthropic은 자체 토크나이저를 공개하지 않는다. 대신 `POST /v1/messages/count_tokens` 엔드포인트를 두어, 보내기 전에 정확한 수를 알고 싶으면 서버에 물어보게 한다. **클라이언트가 혼자서는 정확히 셀 수 없다는 사실이 API 설계에 그대로 드러나 있는 셈이다.** 모델 세대가 바뀌면 토크나이저도 바뀔 수 있어서, 같은 문장의 토큰 수가 모델에 따라 달라진다.

OpenAI는 `tiktoken`(자사 모델용 토큰 계산 라이브러리)을 공개해 두어 로컬 계산이 가능하다. 다만 이건 OpenAI 모델 전용이다. 다른 회사 모델의 토큰 수를 `tiktoken`으로 재면 어긋난다. 그리고 로컬에서 잰 값도 최종값은 아니다. 하네스가 조립한 부분까지는 셀 수 있어도, 서버가 요청을 처리하며 덧붙이는 부분은 클라이언트가 알 수 없다.

Google 계열은 SentencePiece(Google이 만든 오픈소스 토큰화 라이브러리) 계통을 부분 공개한다.

실무 함의는 단순하다. 토큰 수를 근거로 비용을 계산하거나 컨텍스트 예산을 짤 때, 모델을 명시하지 않은 숫자는 의미가 없다. 그리고 정확한 값이 필요하면 제공자가 주는 값을 써야 한다.

### 웹 검색이 실행되는 자리

"최신 버전 찾아줘"라고 입력했을 때 실제로 검색 요청을 인터넷에 쏘는 주체는 둘 중 하나다. 내 컴퓨터에서 도는 하네스이거나, 제공자 서버다. 이 구분이 **클라이언트 도구(client tool)** 와 **서버 도구(server tool)** 다.

**클라이언트 도구**는 모델이 도구를 쓰겠다는 블록(`tool_use`)을 내면, 하네스가 그걸 받아 자기 머신에서 실행하고 결과를 `tool_result`로 되돌려 보내는 방식이다. 파일 읽기, 셸 명령, 파일 편집이 여기 속한다. 도구의 스키마를 제공자가 정의해 두더라도 실행은 클라이언트가 한다.

**서버 도구**는 요청의 `tools` 배열에 선언만 해 두면 제공자 인프라에서 실행되고, 결과가 같은 응답 안에 콘텐츠 블록으로 들어와 있는 방식이다. 하네스는 실행 루프를 돌지 않는다. Anthropic API에서는 웹 검색, 웹 페치, 코드 실행이 여기 속한다.

한 번의 검색 요청을 따라가면 차이가 뚜렷하다.

```
경우 1 – 클라이언트 도구 (하네스가 검색을 실행한다)
  ...
  결과 ──────── tool_result ────────→
                                      토큰화 · 추론
                ←──── 최종 답변 ─────────
  왕복 2회. 검색어는 내가 고른 검색 서비스로 간다.


경우 2 – 서버 도구 (제공자가 검색을 실행한다)

  내 컴퓨터                          제공자 서버
  ────────────                       ────────────
  프롬프트 조립 ──── JSON ──────────→
  (tools에 web_search 선언)           토큰화 · 추론
                                      ★ 제공자가 검색 실행
                                      결과를 컨텍스트에 삽입
                                      추론 계속
                ←─ 검색 기록 + 결과 + 답변을 한 응답에 ─
...
관측되는 차이도 정확히 여기서 갈린다. 클라이언트 도구는 내 머신의 네트워크 로그에 검색 요청이 별도로 찍히고, 훅이나 권한 설정으로 차단할 수 있고, 감사 로그를 내가 남길 수 있다. 서버 도구는 내 머신 로그에 제공자 API 호출 하나만 남고, 하네스에 개입 지점이 없으므로 훅으로 막을 수 없고, 검색어 자체가 제공자에게 전달된다.

그래서 선택 기준이 실제 요구사항에서 나온다. 사내망 문서만 검색해야 하거나, 검색어에 대외비가 섞일 수 있거나, 감사 로그를 직접 보관해야 한다면 클라이언트 도구여야 한다. 설정과 유지보수 비용을 줄이고 왕복 횟수를 아끼는 게 목적이라면 서버 도구가 낫다.

주의할 점이 두 가지 있다.

첫째, 이름이 같아도 어느 쪽인지는 제품마다 다르다. 확인하는 방법은 두 가지다. 요청을 보내는 동안 로컬 네트워크 로그를 보거나, 그 도구가 권한·훅 설정 화면에 나타나는지 본다. 실행 전에 승인 프롬프트가 뜬다면 클라이언트 도구다. 하네스가 실행을 붙잡고 있다는 뜻이기 때문이다.

둘째, MCP로 붙인 도구는 보통 클라이언트 경로를 탄다. 원격 URL의 MCP 서버라도 그 서버를 호출하는 주체가 하네스이기 때문이다. 다만 제공자 API가 원격 MCP 서버에 직접 연결하는 방식도 있으므로, MCP라는 사실만으로 실행 위치가 결정되지는 않는다.

이 절의 요점은 한 문장이다. **같은 프롬프트라도 도구를 어느 쪽에 두느냐에 따라 차단 가능성, 관측 가능성, 데이터가 흘러가는 경로가 전부 달라진다.** 그리고 그건 모델이 정하는 게 아니라 하네스 설계가 정한다.
